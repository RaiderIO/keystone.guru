...
use App\Http\Requests\AdminDungeonRouteIndexRequest;
...
class AdminDungeonRouteController extends Controller
{
    ...
    {
        $dungeonId        = $request->integer('dungeon_id') > 0 ? $request->integer('dungeon_id') : null;
        $publishedStateId = $request->integer('published_state_id') ?: null;
        $author           = $request->string('author')->toString();
        $publicKey        = $request->string('public_key')->toString();

        $query = DungeonRoute::with(['author', 'dungeon', 'publishedState', 'mdtImport'])
