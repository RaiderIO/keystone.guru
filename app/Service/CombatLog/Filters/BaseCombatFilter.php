...
abstract class BaseCombatFilter implements CombatLogParserInterface
{
    ...
    public function __construct(private readonly Collection $resultEvents)
    {
        ...
        $this->unitDefeatedFilter = new UnitDefeatedFilter();
    ...
    public function parse(BaseEvent $combatLogEvent, int $lineNr): bool
    {
        ...
        if ($this->unitDefeatedFilter->parse($combatLogEvent, $lineNr)) {
            if ($this->unitDefeatedFilter->isPlayerDeathEntry($combatLogEvent)) {
                /**
            ...
            if ($combatLogEvent instanceof EncounterEndInterface) {
                ...
                if ($npc !== null) {
                    ...
                    $enemyEngagedEvent = $this->accurateEnemySightings->first(fn(
                        $value,
                        $key,
                    ) => str_contains((string)$key, (string)$npc->id));
            ...
            if ($enemyEngagedEvent instanceof AdvancedCombatLogEvent) {
