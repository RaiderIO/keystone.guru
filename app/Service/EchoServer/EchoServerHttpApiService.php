...
class EchoServerHttpApiService implements EchoServerHttpApiServiceInterface
{
    ...
    public function __construct()
    {
        ...
        $appUrl = trim((string)config('keystoneguru.reverb.url'), '/');

        try {
            $this->_client = new Client([
                // Base URI is used with relative requests
                'base_uri' => sprintf('%s:%s', $appUrl, config('keystoneguru.reverb.port')),
    ...
    private function _doRequest($uri): array
    {
        ...
        if ($this->_client !== null) {
            ...
            $response = $this->_client->get(
                sprintf('apps/%s/%s', config('keystoneguru.reverb.client.app_id'), $uri),
                ['query' => ['auth_key' => config('keystoneguru.reverb.client.key')]],
            );
