 ...
 */
$min                    = $season !== null ? $season->key_level_min : config('keystoneguru.keystone.levels.default_min');
$max                    = $season !== null ? $season->key_level_max : config('keystoneguru.keystone.levels.default_max');
$levelMin               ??= $min;
$levelMax               ??= $max;
$minAnchorKeyLevelWidth ??= 1;
?>
<div class="row g-0 h-100 px-2">
    <div class="col progress h-100" style="border-radius: 0">
        @if($levelMin > $min)
            <div class="progress-bar text-start ps-1" role="progressbar" aria-valuenow="{{ $min }}"
        ...
        @if($levelMax < $max)
            <div class="progress-bar text-end pe-1" role="progressbar" aria-valuenow="{{ $levelMax }}"
