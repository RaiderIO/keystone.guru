 ...
 * The admin-only Developer toggle is the exception: it only appears on pages that pass developer entries.
 *
 * @var string                                  $id
 * @var string                                  $fa
 * @var string                                  $text
 * @var array<int, array<string, mixed>>        $entries      route|modal, modalTab, fa, text, description, strict
 * @var int                                     $columns
 * @var bool                                    $menuEnd
...
<li class="nav-item dropdown">
    ...
    <div class="dropdown-menu ksg-nav-panel {{ $columns === 2 ? 'ksg-nav-panel--2col' : '' }} {{ $menuEnd ? 'dropdown-menu-end' : '' }}"
         aria-labelledby="{{ $id }}">
        @foreach($entries as $entry)
            <a class="dropdown-item ksg-nav-entry {{ $loop->first ? 'ksg-nav-entry--primary' : '' }} {{ isset($entry['route']) ? $isActiveRoute($entry['route'], $entry['strict'] ?? false) : '' }}"
               ...
               @endisset
               @isset($entry['modalTab'])
                   data-modal-tab="{{ $entry['modalTab'] }}"
               @endisset>
