@extends('layouts.sitepage', ['showAds' => false, 'title' => __('view_admin.userreport.list.title')])

@section('header-title')
    {{ __('view_admin.userreport.list.header') }}
@endsection
...
@section('content')
    <table id="admin_user_reports_table" class="tablesorter default_table table-striped">
        <thead>
        <tr>
            <th width="5%">{{ __('view_admin.userreport.list.table_header_id') }}</th>
            <th width="10%">{{ __('view_admin.userreport.list.table_header_author_name') }}</th>
            <th width="10%">{{ __('view_admin.userreport.list.table_header_category') }}</th>
            <th width="40%">{{ __('view_admin.userreport.list.table_header_message') }}</th>
            <th width="10%">{{ __('view_admin.userreport.list.table_header_contact_at') }}</th>
            <th width="10%">{{ __('view_admin.userreport.list.table_header_create_at') }}</th>
            <th width="15%">{{ __('view_admin.userreport.list.table_header_actions') }}</th>
        </tr>
        </thead>
            ...
            <tr>
                ...
                <td>
                    <button class="btn btn-success mark_as_handled_btn" data-id="{{$report->id}}">
                        <i class="fas fa-check-circle"></i> {{ __('view_admin.userreport.list.handled') }}
                    </button>
                </td>
            </tr>
        @endforeach
