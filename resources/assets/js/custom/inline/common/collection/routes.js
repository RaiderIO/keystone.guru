/**
 ...
 @property {Number|null} dungeonId      The pool dungeon of a season set slot; null for the flat and foreign lists.
 @property {boolean} isForeign          The trailing list of routes that do not match the collection.
...
class CommonCollectionRoutes extends InlineCode {
    ...
    _onRemoved(section, removed) {
        ...
        this._request('DELETE', this.options.deleteUrl, [publicKey], function () {
            ...
            // A route that does not match the collection cannot be added back, so there is nothing to undo
            let undo = section.isForeign ? null : function () {
    ...
    _findSectionFor(dungeonRoute) {
        let candidates = this.options.sections.filter(section => !section.isForeign);
