<?php

return [

    'edit' => [
        'title'                   => 'Профиль',
        'profile'                 => 'Профиль',
        'account'                 => 'Аккаунт',
        'patreon'                 => 'Patreon',
        'change_password'         => 'Сменить пароль',
        'privacy'                 => 'Конфиденциальность',
        'reports'                 => 'Жалобы',
        'menu_title'              => '%s\'s профиль',
        'avatar'                  => 'Аватар',
        'avatar_alt'              => 'Аватар пользователя',
        'username'                => 'Имя пользователя',
        'username_title'          => 'Поскольку вы вошли в систему с помощью внешней службы аутентификации, вы можете изменить свое имя пользователя один раз.',
        'email'                   => 'Электронный адрес',
        'region'                  => 'Регион',
        'select_region'           => 'Выбрать регион',
        'show_as_anonymous'       => 'Отображать как аноним',
        'show_as_anonymous_title' => 'Включение этой опции будет отображать вас как анонима при просмотре маршрутов, которые не являются частью какой-либо команды, частью которой вы являетесь.
                                 Для ваших собственных маршрутов и для маршрутов, входящих в состав ваших команд, ваше имя всегда будет видно.',
        'echo_color'                                       => 'Синхронизация редактирования цвета маршрута',
        'echo_color_title'                                 => 'При редактировании маршрута совместно с членом команды этот цвет будет идентифицировать вас.',
        'save'                                             => 'Сохранить',
        'account_delete_consequences'                      => 'Если вы удалите свою учетную запись Keystone.guru, произойдет следующее:',
        'account_delete_consequence_routes'                => 'Маршруты',
        'account_delete_consequence_routes_delete'         => 'Ваш %s маршрут(ы) будут удален.',
        'account_delete_consequence_teams'                 => 'Команды',
        'account_delete_consequence_teams_you_are_removed' => 'Вы будете удалены из этой команды.',
        'account_delete_consequence_teams_new_admin'       => '%s будет назначен администратором этой команды.',
        'account_delete_consequence_teams_team_deleted'    => 'Эта команда будет удалена (вы единственный пользователь в этой команде).',
        'account_delete_consequence_patreon'               => 'Связь между Patreon и Keystone.guru будет прервана. Вы больше не будете получать бонусы Patreon.',
        'account_delete_consequence_reports'               => 'Жалобы',
        'account_delete_consequence_reports_unresolved'    => 'Ваши неразрешенные жалобы (%s) будут удалены.',
        'account_delete_warning'                           => 'Ваша учетная запись будет удалена без возможности восстановления. Пути назад нет  .',
        'account_delete_confirm'                           => 'Удалить мою учетную запись на Keystone.guru',
        'unlink_from_patreon'                              => 'Отменить связь с Patreon',
        'patreon_status_for_admin'                         => 'Вашей учетной записи предоставлены все бонусы :patreon, поскольку вы администратор. Привет, я сам!',
        'patreon_status_granted_manually'                  => 'Вашему аккаунту предоставлены все преимущества :patreon, потому что вы особенный и замечательный, и не позволяйте никому говорить вам обратное! Ваши активные подписки перечислены ниже. Чего-то не хватает? Дайте мне знать как можно скорее в Discord, и я разберусь.',
        'link_to_patreon_success'                          => 'Ваша учетная запись привязана к Patreon. Спасибо!',
        'link_to_patreon_admin'                            => 'Свяжите учетную запись администратора с Patreon, чтобы задачи синхронизации работали корректно!',
        'link_to_patreon'                                  => 'Ссылка на Патреон',
        'link_to_patreon_description'                      => 'Чтобы получить бонусы подписчика Patreon, вам необходимо привязать свою учетную запись к Patreon.',
        'patreon_benefit_table'                            => [
            'header_active'  => 'Активно',
            'header_benefit' => 'Преимущество',
        ],
        'current_password'     => 'Текущий пароль',
        'new_password'         => 'Новый пароль',
        'new_password_confirm' => 'Подтвердить новый пароль',
        'submit'               => 'Подтвердить',
        'ga_cookies_opt_out'   => 'Отключение файлов cookie Google Analytics',
        'ad_free_giveaway'     => [
            'title'                                      => 'Подарок без рекламы',
            'ad_free_giveaway_description_not_available' => 'Подписка на :patreon позволяет вам подарить до :max возможностей просмотра Keystone.guru без рекламы любому участнику команды.',
            'ad_free_giveaway_description_available'     => 'Спасибо за подписку на :patreon! Вы можете подарить еще :current возможностей просмотра Keystone.guru без рекламы любому участнику команды.',
            'table_header_team'                          => 'Команда',
            'table_header_member'                        => 'Участник',
            'table_header_ad_free'                       => 'Без рекламы',
        ],
        'reports_description'              => 'Здесь будут перечислены все маршруты, враги и другие отчеты, которые вы сделали на сайте.',
        'reports_table_header_id'          => 'ID',
        'reports_table_header_category'    => 'Категория',
        'reports_table_header_message'     => 'Сообщение',
        'reports_table_header_created_at'  => 'Дата создания',
        'reports_table_header_status'      => 'Статус',
        'reports_table_action_handled'     => 'Обработано',
        'creator'                          => 'Профиль создателя',
        'creator_description'              => 'Ваш публичный профиль - это то, что видят люди, когда открывают ссылку на ваши маршруты. Добавьте биографию, свяжите свои каналы и закрепите маршруты, которые хотите показать в первую очередь.',
        'creator_bio'                      => 'Биография',
        'creator_bio_placeholder'          => 'Расскажите людям, кто вы и какие маршруты создаете.',
        'creator_bio_help'                 => 'До :max символов. Отображается в верхней части вашего публичного профиля.',
        'creator_socials'                  => 'Ссылки',
        'creator_socials_help'             => 'Принимаются только https-ссылки на соответствующий сайт. Оставьте поле пустым, чтобы удалить эту ссылку.',
        'creator_pinned_routes'            => 'Закрепленные маршруты',
        'creator_pinned_routes_help'       => 'Выберите до :max собственных маршрутов, чтобы выделить их в верхней части вашего профиля. Неопубликованные маршруты остаются скрытыми от других, даже если закреплены.',
        'creator_pinned_routes_empty'      => 'Закреплённых маршрутов пока нет.',
        'creator_pinned_routes_none'       => 'Вы еще не создали ни одного маршрута, поэтому закреплять нечего.',
        'creator_pinned_collections'       => 'Закрепленные коллекции',
        'creator_pinned_collections_none'  => 'Вы еще не создали ни одной коллекции.',
        'creator_pinned_collections_help'  => 'Удерживайте ctrl (или cmd), чтобы выбрать до :max коллекций для выделения над Вашими маршрутами. Коллекции, не видимые публично, остаются скрытыми от других, даже если они закреплены.',
        'creator_pinned_collections_empty' => 'Закреплённых коллекций пока нет.',
        'creator_directory'                => 'Каталог создателей',
        'creator_directory_hide'           => 'Скрыть меня из каталога создателей',
        'creator_directory_hide_help'      => 'Создатели с опубликованными маршрутами автоматически отображаются в каталоге. Отметьте это, чтобы исключить свой профиль из него - ваши маршруты в любом случае останутся опубликованными.',
        'creator_save'                     => 'Сохранить профиль создателя',
        'creator_view_public_profile'      => 'Просмотреть мой публичный профиль',
    ],
    'favorites' => [
        'title' => 'Избранное',
    ],
    'overview' => [
        'title'          => 'Обзор',
        'route_coverage' => 'Таблица готовности маршрутов',
        'route_overview' => 'Список маршрутов',
    ],
    'routes' => [
        'title' => 'Мои маршруты',
    ],
    'tags' => [
        'title'                             => 'Мои теги',
        'header'                            => 'Мои теги',
        'description'                       => 'Функция маркировки позволяет вам организовать свои маршруты так, как вы считаете нужным. Вы можете добавлять теги к маршрутам, просматривая Действия для каждого маршрута в %s. Здесь вы можете управлять тегами для своих маршрутов. Никто другой не сможет просматривать ваши теги - для маршрутов, прикрепленных к группе, вы можете управлять отдельным набором тегов только для этой группы, посетив раздел Теги при просмотре своей команды.',
        'link_your_personal_route_overview' => 'Просмотр вашего маршрута',
    ],
    'view' => [
        'title'              => '%s\'s маршруты',
        'header'             => '%s\'s маршруты',
        'pinned_collections' => 'Закрепленные коллекции',
        'pinned_routes'      => 'Закрепленные маршруты',
        'all_routes'         => 'Все маршруты',
        'route_count'        => '{0} Нет опубликованных маршрутов|{1} :count опубликованный маршрут|[2,*] :count опубликованных маршрутов',
        'member_since'       => 'Создает маршруты с :date',
        'social_link'        => 'Посетить :platform',
        'platform'           => [
            'twitch'    => 'Twitch',
            'youtube'   => 'YouTube',
            'x'         => 'X',
            'discord'   => 'Discord',
            'bluesky'   => 'Bluesky',
            'tiktok'    => 'TikTok',
            'instagram' => 'Instagram',
            'patreon'   => 'Patreon',
            'website'   => 'Веб-сайт',
        ],
    ],

];
