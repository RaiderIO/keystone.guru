...
return [
    ...
    'mdt' => [
        'io' => [
            'export_string' => [
                ...
                'unable_to_place_kill_zone_spells_note'             => 'Заклинания, назначенные этому пулу, не были экспортированы: они экспортируются как заметка MDT рядом с пулом, но у этого пула нет ни врагов, ни области зачистки, где можно было бы разместить эту заметку.',
            ],
            'import_string' => [
                'category' => [
                ...
                'limit_reached_arrows'                                 => 'Не удалось импортировать маршрут: больше максимально допустимых :limit стрелок.',
                ...
                'unable_to_find_awakened_obelisk_enemy'                => 'Не удалось импортировать Пробужденный обелиск :name, его врага не удалось определить для вашей комбинации подземелья и недели.',
    ...
    'combatlog' => [
        'enemy_failure_analysis' => [
            'verdict' => [
                'npc_not_mapped'       => 'НПЦ не размечен',
                'no_enemy_in_range'    => 'Нет врагов в радиусе',
                'enemies_exhausted'    => 'В игре больше, чем в разметке',
                'wrong_floor_artifact' => 'Вероятно, артефакт неверного уровня',
            ],
            'suggestion' => [
                'npc_not_mapped'       => ':npc вообще отсутствует в этой версии карты, но здесь с ним вступали в бой :count раз в :routes маршрутах. Добавьте его (или его пак) в этой точке.',
                'no_enemy_in_range'    => 'Ближайший размеченный :npc (враг :enemy_id) находится в :distance ярд. отсюда, за пределами радиуса вступления в бой :range ярд. Скорее всего, здесь не хватает пака :npc или он размечен не в том месте.',
                'enemies_exhausted'    => 'Размеченных :npc в радиусе: :enemies, но на каждом маршруте здесь всё равно происходит в среднем около :avg сбоев — вероятно, в этом паке в игре больше :npc, чем в разметке.',
