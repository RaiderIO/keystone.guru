<?php

return [

    'view_route_not_published'                            => 'Cet itinéraire n\'est pas publié et ne peut pas être consulté. Veuillez demander à l\'auteur de publier cet itinéraire pour le visualiser.',
    'present_route_not_published'                         => 'Cet itinéraire n\'est pas publié et ne peut pas être présenté. Veuillez demander à l\'auteur de publier cet itinéraire pour le présenter.',
    'embed_route_not_published'                           => 'Cet itinéraire n\'est pas publié et ne peut pas être consulté. Veuillez demander à l\'auteur de publier cet itinéraire pour le visualiser.',
    'embed_route_sandbox_not_allowed'                     => 'Les itinéraires temporaires ne peuvent pas être intégrés.',
    'publish_not_all_required_enemies_killed'             => 'Impossible de modifier les paramètres de partage : tous les ennemis requis n\'ont pas été tués.',
    'publish_route_is_upgrade_draft'                      => 'Un brouillon de mise à niveau ne peut pas être partagé seul. Appliquez-le à la route qu\'il met à niveau pour publier vos modifications.',
    'apply_upgrade_route_not_upgrade_draft'               => 'Cette route n\'est pas un brouillon de mise à niveau, il n\'y a donc rien à appliquer.',
    'apply_upgrade_original_route_deleted'                => 'La route que ce brouillon met à niveau n\'existe plus.',
    'apply_upgrade_draft_not_all_required_enemies_killed' => 'Impossible d\'appliquer ce brouillon de mise à niveau : la nouvelle version de mapping requiert des ennemis qui n\'ont pas été tués dans le brouillon. Tuez les ennemis manquants avant d\'appliquer, ou abandonnez le brouillon.',
    'discard_upgrade_route_not_upgrade_draft'             => 'Cette route n\'est pas un brouillon de mise à niveau, il n\'y a donc rien à abandonner.',
    'add_kill_zone_limit_reached'                         => 'Impossible d\'ajouter plus de :limit zones de combat à un seul itinéraire.',
    'add_brushline_limit_reached'                         => 'Impossible d\'ajouter plus de :limit lignes dessinées à main levée à un seul itinéraire.',
    'add_path_limit_reached'                              => 'Impossible d\'ajouter plus de :limit chemins à un seul itinéraire.',
    'add_arrow_limit_reached'                             => 'Impossible d\'ajouter plus de :limit flèches à une seule route.',
    'add_map_icon_limit_reached'                          => 'Impossible d\'ajouter plus de :limit icônes de carte à un seul itinéraire.',
    'schedule_publish_route_not_in_team'                  => 'Impossible de planifier la publication : cette route n\'est assignée à aucune équipe.',
    'game_version_not_active'                             => 'Cette version du jeu n\'est plus active.',
    'expansion_not_active'                                => 'Cette extension n\'est plus active.',
    'dungeon_not_active'                                  => 'Ce donjon n\'est plus actif.',
    'season_not_active'                                   => 'Cette saison est introuvable ou n\'est plus active.',

    'claim_route_not_claimable'                   => 'Cette route a déjà un auteur et ne peut pas être revendiquée.',
    'make_role_only_super_admins_may_grant_admin' => 'Seuls les super administrateurs peuvent accorder ou révoquer le rôle d\'administrateur.',
    'create_global_map_icon_admin_only'           => 'Seuls les administrateurs peuvent créer des icônes de carte qui ne sont rattachées à aucune route.',
    'update_map_icon_admin_only'                  => 'Seuls les administrateurs peuvent modifier des icônes de carte qui ne sont rattachées à aucune route ou équipe.',
    'update_team_map_icon_collaborator_only'      => 'Seuls les collaborateurs d’une équipe peuvent modifier les icônes de carte de cette équipe.',
    'delete_map_icon_admin_only'                  => 'Seuls les administrateurs peuvent supprimer des icônes de carte qui ne sont rattachées à aucune route.',

];
