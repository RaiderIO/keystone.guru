...
final class NpcScalingFactorTest extends PublicTestCase
{
    ...
    public function getScalingFactor_givenKeyLevel_matchesMeasuredTrashHealth(int $keyLevel, float $measuredFactor): void
    {
        // Arrange
        $npc = $this->npc(NpcClassification::NPC_CLASSIFICATION_NORMAL);

        // Act
        $factor = $npc->getScalingFactor($keyLevel);
     ...
     * 2,918,930 (+7), 3,132,985 (+8), 3,347,040 (+9), 3,580,555 (+10), 4,320,017 (+12).
     *
    ...
    public static function getScalingFactor_givenKeyLevel_matchesMeasuredTrashHealth_DataProvider(): array
    {
        return [
            ...
            '+7 (round2(1.07^6) * 1.2 fortified)' => [7, 1.80],
            '+8'                                  => [8, 1.932],
            '+9'                                  => [9, 2.064],
            '+10'                                 => [10, 2.208],
    ...
    public function getScalingFactor_givenKeyLevel_matchesMeasuredBossHealth(int $keyLevel, float $measuredFactor): void
    {
        // Arrange
     ...
     * 31,783,898 (+6), 34,054,178 (+7), 36,551,484 (+8), 39,048,788 (+9), 52,216,404 (+10), 63,000,225 (+12).
     *
     * @return array<string, array{int, float}>
     */
    public static function getScalingFactor_givenKeyLevel_matchesMeasuredBossHealth_DataProvider(): array
    {
        return [
            ...
            '+5'                                         => [5, 1.2576],
            '+6'                                         => [6, 1.344],
            '+7 (no fortified on bosses)'                => [7, 1.44],
            '+8'                                         => [8, 1.5456],
            '+9'                                         => [9, 1.6512],
    ...
    #[Test]
    public function getScalingFactor_givenFortifiedAffixBelowThreshold_appliesFortifiedToTrashOnly(): void
    {
        // Arrange
        ...
        // Act
        $trashFactor = $trash->getScalingFactor(4, [Affix::AFFIX_FORTIFIED]);
        $bossFactor  = $boss->getScalingFactor(4, [Affix::AFFIX_FORTIFIED]);

        // Assert
        $this->assertEqualsWithDelta(1.23 * 1.2 * 0.95, $trashFactor, 0.0001);
        $this->assertEqualsWithDelta(1.23 * 0.96, $bossFactor, 0.0001);
    }

    #[Test]
    public function getScalingFactor_givenTyrannicalAffixBelowThreshold_appliesTyrannicalToBossesOnly(): void
    {
        // Arrange
        $trash = $this->npc(NpcClassification::NPC_CLASSIFICATION_NORMAL);
        $boss  = $this->npc(NpcClassification::NPC_CLASSIFICATION_BOSS);
        ...
        // Act
        $trashFactor = $trash->getScalingFactor(6, [Affix::AFFIX_TYRANNICAL]);
        $bossFactor  = $boss->getScalingFactor(6, [Affix::AFFIX_TYRANNICAL]);

        // Assert
        $this->assertEqualsWithDelta(1.40, $trashFactor, 0.0001);
        $this->assertEqualsWithDelta(1.40 * 1.25 * 0.96, $bossFactor, 0.0001);
    }
    ...
    {
        // Arrange - Demon Fly's seeded base and its observed +8 max HP
        ...
        // Act
        $health = $npc->calculateHealthForKey($gameVersion, 8);
