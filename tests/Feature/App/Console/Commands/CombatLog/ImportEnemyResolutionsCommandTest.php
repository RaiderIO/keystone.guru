<?php

namespace Tests\Feature\App\Console\Commands\CombatLog;

use App\Models\CombatLog\CombatLogRouteEnemyResolution;
use App\Models\Dungeon;
use App\Models\Enemy;
use App\Models\Floor\Floor;
use App\Models\Mapping\MappingVersion;
use Illuminate\Http\Client\ConnectionException;
use Illuminate\Http\Client\Request;
use Illuminate\Support\Facades\File;
use Illuminate\Support\Facades\Http;
use PHPUnit\Framework\Attributes\Group;
use PHPUnit\Framework\Attributes\Test;
use Tests\Feature\Traits\ProvidesDungeon;
use Tests\TestCases\PublicTestCase;

#[Group('CombatLog')]
#[Group('ImportEnemyResolutionsCommand')]
final class ImportEnemyResolutionsCommandTest extends PublicTestCase
{
    use ProvidesDungeon;

    private const string BASE_URL = 'https://ksg-test.example';

    private Dungeon $dungeon;

    private Floor $floor;

    private MappingVersion $mappingVersion;

    private Enemy $enemy;

    private string $credentialsFile;

    private string $postBodiesDir;

    #[\Override]
    protected function setUp(): void
    {
        parent::setUp();

        /** @var Enemy $enemy */
        [$this->dungeon, $this->mappingVersion, $enemy] = $this->findDungeon(
            facadeEnabled: false,
            resolve:       static fn(Dungeon $dungeon, MappingVersion $mappingVersion) => $mappingVersion->enemies()
                ->whereIn('floor_id', $dungeon->floors()->where('facade', 0)->select('id'))
                ->first(),
        );
        $this->enemy = $enemy;

        /** @var Floor $floor */
        $floor       = $this->dungeon->floors()->where('id', $enemy->floor_id)->firstOrFail();
        $this->floor = $floor;

        config(['keystoneguru.remote_hosts.production.base_url' => self::BASE_URL]);

        // The combatlog test database is persistent and this dungeon is a seeded one, so it can carry rows that have
        // nothing to do with this test - tearDown() must leave those alone
        $this->preExistingResolutionIds = CombatLogRouteEnemyResolution::query()
            ->where('dungeon_id', $this->dungeon->id)
            ->pluck('id')
            ->all();

        $this->credentialsFile = storage_path(sprintf('framework/testing/import-enemy-resolutions-%s.txt', uniqid()));
        $this->postBodiesDir   = storage_path(sprintf('framework/testing/import-enemy-resolutions-bodies-%s', uniqid()));
        File::ensureDirectoryExists(dirname($this->credentialsFile));
        File::put($this->credentialsFile, "admin@example.com:secret\n");
    }

    /** @var array<int, int> Rows of the dungeon that are not this test's to delete */
    private array $preExistingResolutionIds = [];

    #[\Override]
    protected function tearDown(): void
    {
        try {
            File::delete($this->credentialsFile);
            File::deleteDirectory($this->postBodiesDir);
            CombatLogRouteEnemyResolution::query()
                ->where('dungeon_id', $this->dungeon->id)
                ->whereNotIn('id', $this->preExistingResolutionIds)
                ->delete();
        } finally {
            parent::tearDown();
        }
    }

    #[Test]
    public function handle_givenTwoPages_replacesRowsOfThatSourceAndStopsWhenHasMoreIsFalse(): void
    {
        // Arrange — a stale production-imported row that must disappear, a locally recorded row and a staging-imported
        // row that must both survive, and two remote pages
        $stale   = $this->createLocalResolution(['npc_id' => 1, 'source' => 'production']);
        $local   = $this->createLocalResolution(['npc_id' => 2]);
        $staging = $this->createLocalResolution(['npc_id' => 3, 'source' => 'staging']);

        Http::fake([
            self::BASE_URL . '/api/v1/combatlog/enemy-resolutions/*' => function (Request $request) {
                $afterId = (int)($request->data()['after_id'] ?? 0);

                return $afterId === 0
                    ? Http::response(self::page([$this->remoteRow(10, 501, 'KEY-A'), $this->remoteRow(11, 502, 'KEY-B')], 11, true))
                    : Http::response(self::page([$this->remoteRow(12, 503, 'KEY-A')], null, false));
            },
        ]);

        // Act
        $this->artisan('combatlog:importenemyresolutions', [
            'dungeon'            => $this->dungeon->key,
            '--credentials-file' => $this->credentialsFile,
        ])->assertSuccessful();

        // Assert — the three remote rows tagged with their source, the stale production row gone, local + staging kept
        $imported = CombatLogRouteEnemyResolution::query()->where('dungeon_id', $this->dungeon->id)->where('source', 'production')->orderBy('id')->get();
        $this->assertCount(3, $imported);
        $this->assertNull(CombatLogRouteEnemyResolution::find($stale->id));
        $this->assertNotNull(CombatLogRouteEnemyResolution::find($local->id));
        $this->assertNotNull(CombatLogRouteEnemyResolution::find($staging->id));
        $this->assertSame([501, 502, 503], $imported->pluck('npc_id')->all());
        $this->assertSame($this->floor->id, $imported->first()->floor_id);

        // The columns the failure import does not carry come across as well
        $this->assertSame($this->enemy->id, $imported->first()->enemy_id);
        $this->assertSame(-14.5, $imported->first()->enemy_lat);
        $this->assertSame(36.75, $imported->first()->enemy_lng);
        $this->assertSame(55.5, $imported->first()->distance);
        $this->assertSame(66.5, $imported->first()->weighted_distance);

        Http::assertSentCount(2);
        Http::assertSent(static fn(Request $request) => $request->hasHeader('Authorization') && ($request->data()['after_id'] ?? null) === 11);
    }

    #[Test]
    public function handle_givenMissingCredentialsFile_returnsFailureWithoutTouchingLocalRows(): void
    {
        // Arrange
        $existing = $this->createLocalResolution();
        Http::fake();

        // Act
        $this->artisan('combatlog:importenemyresolutions', [
            'dungeon'            => $this->dungeon->key,
            '--credentials-file' => $this->credentialsFile . '.missing',
        ])->assertFailed();

        // Assert
        $this->assertNotNull(CombatLogRouteEnemyResolution::find($existing->id));
        Http::assertNothingSent();
    }

    #[Test]
    public function handle_givenFirstPageFails_leavesLocalRowsUntouched(): void
    {
        // Arrange
        $existing = $this->createLocalResolution();
        Http::fake([
            self::BASE_URL . '/api/v1/combatlog/enemy-resolutions/*' => Http::response(['error' => 'Unauthenticated'], 401),
        ]);

        // Act
        $this->artisan('combatlog:importenemyresolutions', [
            'dungeon'            => $this->dungeon->key,
            '--credentials-file' => $this->credentialsFile,
        ])->assertFailed();

        // Assert
        $this->assertNotNull(CombatLogRouteEnemyResolution::find($existing->id));
    }

    #[Test]
    public function handle_givenUnreachableHost_returnsFailureWithoutTouchingLocalRows(): void
    {
        // Arrange
        $existing = $this->createLocalResolution();
        Http::fake(static fn() => throw new ConnectionException('cURL error 6: Could not resolve host'));

        // Act
        $this->artisan('combatlog:importenemyresolutions', [
            'dungeon'            => $this->dungeon->key,
            '--credentials-file' => $this->credentialsFile,
        ])->assertFailed();

        // Assert
        $this->assertNotNull(CombatLogRouteEnemyResolution::find($existing->id));
    }

    #[Test]
    public function handle_givenMappingVersionOption_replacesOnlyThatMappingVersionAndPassesItOn(): void
    {
        // Arrange — one production-imported row in the targeted mapping version (replaced) and one in another (kept)
        $targeted = $this->createLocalResolution(['mapping_version_id' => $this->mappingVersion->id, 'source' => 'production']);
        $other    = $this->createLocalResolution(['mapping_version_id' => PHP_INT_MAX, 'source' => 'production']);

        Http::fake([
            self::BASE_URL . '/api/v1/combatlog/enemy-resolutions/*' => Http::response(self::page([$this->remoteRow(20, 601, null)], null, false)),
        ]);

        // Act
        $this->artisan('combatlog:importenemyresolutions', [
            'dungeon'            => $this->dungeon->key,
            '--credentials-file' => $this->credentialsFile,
            '--mapping-version'  => $this->mappingVersion->id,
        ])->assertSuccessful();

        // Assert
        $this->assertNull(CombatLogRouteEnemyResolution::find($targeted->id));
        $this->assertNotNull(CombatLogRouteEnemyResolution::find($other->id));
        $this->assertSame(1, CombatLogRouteEnemyResolution::query()->where('dungeon_id', $this->dungeon->id)->where('npc_id', 601)->count());
        Http::assertSent(fn(Request $request) => ($request->data()['mapping_version_id'] ?? null) === $this->mappingVersion->id);
    }

    #[Test]
    public function handle_givenMinDistanceOption_passesItOnAsMinDistanceQueryParameter(): void
    {
        // Arrange
        Http::fake([
            self::BASE_URL . '/api/v1/combatlog/enemy-resolutions/*' => Http::response(self::page([$this->remoteRow(40, 801, null)], null, false)),
        ]);

        // Act
        $this->artisan('combatlog:importenemyresolutions', [
            'dungeon'            => $this->dungeon->key,
            '--credentials-file' => $this->credentialsFile,
            '--min-distance'     => '25.5',
        ])->assertSuccessful();

        // Assert
        Http::assertSent(static fn(Request $request) => ($request->data()['min_distance'] ?? null) === 25.5);
    }

    #[Test]
    public function handle_givenDownloadPostBodiesDir_writesOneFilePerDistinctRouteAndSkipsRoutesWithout(): void
    {
        // Arrange — two rows of route KEY-A, one of KEY-B (which has no stored body), one without a route
        Http::fake([
            self::BASE_URL . '/api/v1/combatlog/enemy-resolutions/*' => Http::response(self::page([
                $this->remoteRow(30, 701, 'KEY-A'),
                $this->remoteRow(31, 701, 'KEY-A'),
                $this->remoteRow(32, 702, 'KEY-B'),
                $this->remoteRow(33, 703, null),
            ], null, false)),
            self::BASE_URL . '/api/v1/combatlog/route/KEY-A/post-body' => Http::response('{"npcs":[]}', 200, ['Content-Type' => 'application/json']),
            self::BASE_URL . '/api/v1/combatlog/route/KEY-B/post-body' => Http::response(['error' => 'none'], 404),
        ]);

        // Act
        $this->artisan('combatlog:importenemyresolutions', [
            'dungeon'                => $this->dungeon->key,
            '--credentials-file'     => $this->credentialsFile,
            '--download-post-bodies' => $this->postBodiesDir,
        ])->assertSuccessful();

        // Assert
        $this->assertSame('{"npcs":[]}', File::get($this->postBodiesDir . '/KEY-A.json'));
        $this->assertFalse(File::exists($this->postBodiesDir . '/KEY-B.json'));
        // 1 resolutions page + 2 post body requests (KEY-A once despite two rows, KEY-B once)
        Http::assertSentCount(3);
    }

    /**
     * @param array<string, mixed> $attributes
     */
    private function createLocalResolution(array $attributes = []): CombatLogRouteEnemyResolution
    {
        return CombatLogRouteEnemyResolution::create(array_merge([
            'dungeon_id'         => $this->dungeon->id,
            'floor_id'           => $this->floor->id,
            'mapping_version_id' => $this->mappingVersion->id,
            'npc_id'             => null,
            'enemy_id'           => $this->enemy->id,
            'lat'                => -50.0,
            'lng'                => 100.0,
            'enemy_lat'          => -60.25,
            'enemy_lng'          => 110.5,
            'distance'           => 42.5,
            'weighted_distance'  => 42.5,
        ], $attributes));
    }

    /**
     * @return array<string, mixed>
     */
    private function remoteRow(int $id, int $npcId, ?string $publicKey): array
    {
        return [
            'id'                       => $id,
            'dungeon_id'               => 999999,
            'floor_id'                 => $this->floor->id,
            'mapping_version_id'       => $this->mappingVersion->id,
            'npc_id'                   => $npcId,
            'enemy_id'                 => $this->enemy->id,
            'dungeon_route_id'         => $publicKey === null ? null : $id * 10,
            'dungeon_route_public_key' => $publicKey,
            'lat'                      => -12.5,
            'lng'                      => 34.25,
            'enemy_lat'                => -14.5,
            'enemy_lng'                => 36.75,
            'distance'                 => 55.5,
            'weighted_distance'        => 66.5,
            'created_at'               => '2026-08-20T10:00:00+00:00',
        ];
    }

    /**
     * @param  array<int, array<string, mixed>> $rows
     * @return array<string, mixed>
     */
    private static function page(array $rows, ?int $nextAfterId, bool $hasMore): array
    {
        return [
            'data' => $rows,
            'meta' => [
                'count'         => count($rows),
                'next_after_id' => $nextAfterId,
                'has_more'      => $hasMore,
            ],
        ];
    }
}
