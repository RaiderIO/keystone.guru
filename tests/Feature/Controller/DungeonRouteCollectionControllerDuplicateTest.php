...
final class DungeonRouteCollectionControllerDuplicateTest extends PublicTestCase
{
    ...
    public function create_givenOneOfTheUsersRoutes_startsFromItsGameVersionAndSeasonWithTheRouteSelected(): void
    {
        ...
        $this->assertSame([$dungeonRoute->id], $this->preselectedDungeonRouteIds($response));
        // Picking another season rebuilds the routes section from this same form, which must keep the route
        $this->assertSame(['dungeon_route' => $dungeonRoute->public_key], $response->viewData('startFromQuery'));
    }
    ...
    public function create_givenARouteOfAnInactiveSeason_offersThatSeasonSelected(): void
    {
        ...
        $this->assertContains($season->id, $response->viewData('seasons')->pluck('id')->all());
        $this->assertSame($season->id, $response->viewData('selectedSeason')?->id);
        $this->assertMatchesRegularExpression(
            sprintf('/id="season_id_%d"[^>]*\schecked>/', $season->id),
            (string)$response->getContent(),
        );
    }

    #[Test]
    public function create_givenAFailedSaveAfterChangingTheSeason_keepsTheChosenSeason(): void
    {
        // Arrange - the form was opened from a route of one season, the user picked another one and the save failed
        $owner        = $this->createUser();
        $routeSeason  = $this->createRetailSeason();
        $chosenSeason = $this->createRetailSeason();
        $dungeonRoute = $this->createRoute($owner, $this->retailMappingVersion(), $routeSeason);

        // Act
        $response = $this->actingAs($owner)
            ->withSession(['_old_input' => ['season_id' => (string)$chosenSeason->id, 'name' => '']])
            ->get(route('collections.new', ['dungeon_route' => $dungeonRoute->public_key]));

        // Assert
        $response->assertOk();
        $this->assertSame($chosenSeason->id, $response->viewData('selectedSeason')?->id);
    }
    ...
    public function create_givenARouteOfAGameVersionWithoutSeasons_startsFreeForm(): void
    {
        // Arrange
        $owner = $this->createUser();
        $owner->update(['game_version_id' => $this->gameVersionWithoutSeasons()->id]);
        ...
        // Assert
        $response->assertOk();
        $this->assertSame($this->gameVersionWithoutSeasons()->id, $response->viewData('selectedGameVersion')->id);
    ...
    public function create_givenATag_preselectsOnlyTheTaggedRoutesOfTheChosenSeason(): void
    {
        ...
        // Assert
        $response->assertOk();
        $this->assertSame([$tagged->id], $this->preselectedDungeonRouteIds($response));
    ...
    public function create_givenATagAndAFreeFormCollection_preselectsTheTaggedRoutesOfEverySeason(): void
    {
        ...
        $response->assertOk();
        $this->assertNull($response->viewData('selectedSeason'));
        $this->assertSame([$alpha->id, $bravo->id], $this->preselectedDungeonRouteIds($response));
    ...
    /**
     * The routes the new-collection form starts with, gathered from the sections it renders them in.
     *
     * @param  TestResponse<Response> $response
     * @return array<int, int>
     */
    private function preselectedDungeonRouteIds(TestResponse $response): array
    {
        /** @var Collection<int, DungeonRouteCollectionGroup> $editSections */
        $editSections = $response->viewData('editSections');

        return $editSections
            ->flatMap(static fn(DungeonRouteCollectionGroup $editSection): Collection => $editSection->dungeonRoutes)
            ->map(static fn(DungeonRoute $dungeonRoute): int => $dungeonRoute->id)
            ->unique()
            ->values()
            ->all();
    }
