...
final class ExampleEnvironmentParityTest extends TestCase
{
    ...
    public function dockerComposeOnlyVariables_givenTheExceptionList_areNeverReadByTheApplication(): void
    {
        ...
        // moment one gains a config/ consumer it has to be declared in every file like anything else
        $configContents = '';
        ...
        $consumed = array_values(array_filter(
            ...
            static fn(string $variable): bool => str_contains($configContents, $variable),
        ));

        // Assert
        $this->assertSame([], $consumed, 'a docker-compose-only variable gained a config/ consumer');
    }
